...
TypedExport.determineType = function(filePath, className) {
  ...
  // Classnames should end with the titleized type
...
TypedExport.filePathForClassname = function(className, type, filePath, exportFiles) {
  ...
  if (!className && filePath !== 'application.js') {
    ...
    if (['controller', 'route', 'model'].indexOf(type) !== -1 && path.dirname(filePath) !== TypedExport.pluralizeType(type)) {
