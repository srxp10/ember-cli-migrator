...
function EmberMigrator(options){
  ...
  // If source files to stay
  this.keepSource = options.keepSource;
  if (this.keepSource) {
    this.moveOrCopyCmd =  "cp ";
    this.operationChalkText = "Copy";
    this.operationText = " Copying ";
  }
  // Ignore directories
...
EmberMigrator.prototype.run = function EmberMigrator_run(){
  ...
  Object.keys(this.splitFiles).forEach(function(key) {
    ...
    if(fs.existsSync(file.oldFileName)) {
      ...
      if (outputPath !== file.oldFileName && !this.testing) {
        this.writeLine(chalk.green(this.operationChalkText) + this.operationText + file.oldFileName + ' to ' + outputPath);
        execSync(this.moveOrCopyCmd + file.oldFileName + " " + outputPath);
      }
    }
  }, this);
  ...
  hbsFiles.forEach(function (filePath) {
    ...
    if (fullPath === outputFile) {
      ...
      this.writeLine(chalk.green(this.operationChalkText) + this.operationText + filePath + ' to ' + outputFile);
      mkdirp.sync(outputFolder);
  ...
  if (!this.testing && !this.keepSource) {
    ...
    if (namedTypes.ExpressionStatement.check(node) &&
      ...
      flattenedNodes.forEach(function(flatNode) {
        ...
        if (namedTypes.MemberExpression.check(expression.left) &&
            ...
            (expression.left.object.name === "window" && expression.left.property.name === this.rootAppName))) {

          className = expression.left.property.name;
          addTypedNode(flatNode, filePath, className);
          isNodeStored = true;
        }
      }.bind(this));
      ...
      if (namedTypes.AssignmentExpression.check(node.expression) &&
        ...
        if(namedTypes.Identifier.check(node.expression.right)){
          addTypedNode(node, filePath, node.expression.right.name);
          isNodeStored = true;
        }
      }
...
EmberMigrator.prototype.convertFile = function(typedExport){
  ...
  var imports = Object.keys(visitor.imports)
    ...
    .map(function(key){
      ...
      if (importFilename[0] === path.sep) { importFilename = importFilename.substring(1); } // skip leading / from file path
